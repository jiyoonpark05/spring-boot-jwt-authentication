...
public class JwtTokenProvider {
    ...
    // Get secret value from application.yml and save to key
    ...
    // Generate Access Token, Refresh Token based on Member info
    public JwtToken generateToken(Authentication authentication) {
        ...
        String accessToken = Jwts.builder().setSubject(authentication.getName())
                .claim("auth",authorities).setExpiration(accessTokenExpiresIn)
    ...
    // Decryption JWT token, get data from token
    public Authentication getAuthentication(String accessToken) {
        Claims claims = parseClaims(accessToken);

        if (claims.get("auth") ==  null) {
            throw new RuntimeException("no Authentication data");
        }

        // get auth data from claim
        Collection<? extends GrantedAuthority> authorities = Arrays.stream(claims.get("auth").toString().split(","))
                .map(SimpleGrantedAuthority::new).collect(Collectors.toList());

        // Make UserDetail (interface, User) Object
